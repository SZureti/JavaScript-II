...
let runnersTotal = runners.reduce((ticketPriceTotal, reduction) => ticketPriceTotal + reduction.donation, 0);
...
let mediumShirts = [];
for(let i = 0; i < runners.length; i++){
    if(runners[i].shirt_size === "M"){
        mediumShirts.push(runners[i]);
    }
}
...
// Problem 3
// Create a list of all of the runners shirts sizes that are equal to small, medium and large. 

let shirts = [];

for(let i = 0; i < runners.length; i++){
    if(runners[i].shirt_size === "L", "M", "S"){
        shirts.push(runners[i]);
    }
}
